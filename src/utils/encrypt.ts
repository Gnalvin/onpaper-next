// https://github.com/cotag/ts-md5/blob/master/src/md5.ts
...
export function encryptParams(params: { [key: string]: any }, filter = '') {
  // 将请求参数的key提取出来，并排好序
  ...
  for (let i = 0; i < newKeys.length; i++) {
    const k = newKeys[i]
    originStr += k + '=' + params[k] + '&'
  }

  // 第一次计算
  const key = Md5.hashStr(import.meta.env.VITE_MD5_SIGN)
  originStr += 'key=' + key

  // 计算md5值
  const sign = Md5.hashStr(originStr)
