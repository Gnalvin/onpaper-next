...
import { isFunction } from '../utils/verify-type'
import { throttle } from 'lodash-unified'
import { getScrollContainer, getOffsetTopDistance } from '../utils/scroll'
...
const getScrollOptions = (
  ...
  return Object.entries(attributes).reduce((acm, [name, option]) => {
    ...
    const attrVal = el.getAttribute(`lazy-loading-${name}`) as string
...
> = {
  async mounted(el, binding) {
    ...
    if (!isFunction(cb)) {
      console.warn(SCOPE, "'v-lazy-loading' binding value must be a function")
      return
    }
